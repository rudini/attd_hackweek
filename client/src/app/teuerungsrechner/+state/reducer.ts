...
function berechneResultat(state: TeuerungsrechnerStore): Option<ResultModel> {
    const indexMap = state.datenLaden.data.map(x =>
        x.timeDimension.reduce(
            (acc, curr) => ({
                ...acc,
                [curr.id]: curr.name,
            }),
    ...
    const startdatum = state.datenLaden.data
        .map(x => x.timeDimension.filter(f => state.parameters.startdatum.map(s => s === f.name).getOrElse(false)))
        .mapNullable(arr => arr[0])
        .map(i => i.id)
        .getOrElse(-1);

    const zieldatum = state.datenLaden.data
        .map(x => x.timeDimension.filter(f => state.parameters.zieldatum.map(s => s === f.name).getOrElse(false)))
        .mapNullable(arr => arr[0])
        .map(i => i.id)
        .getOrElse(-1);
    ...
    const hasAllValues = veraenderung
        .chain(v => zielbetrag)
        .chain(z => indexe)
        .isSome();
    return !hasAllValues
